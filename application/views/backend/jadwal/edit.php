                        ...
                        <form class="user" method="post" action="<?= base_url('C_jadwal/update') ?>">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-control" id="kode_pju" name="kode_pju"
                                        value="<?php echo $data_pju->kode_pju; ?>">
                                        <option value="">-Pilih Satu-</option>

                                        <?php if ($jd->kode_pju == $jd->kode_pju) : ?>

                                        <option value="<?php echo $jd->kode_pju ?>" selected>
                                            <?php echo $jd->kode_pju ?>
                                        </option>

                                        <?php else : ?>
                                        <option value="<?php echo $jd->kode_pju ?>"><?php echo $jd->kode_pju ?>
                                        </option>
                                        <?php endif; ?>
                ...
                success: function(data) {
                    ...
                    $.each(data, function(key, value) {
                        if (kode_pju == value.kode_pju) {
                            $('select[name="kode_pju"]').append('<option value="' +
                                value.kode_pju + '" selected>' + value
                                .kode_pju + '</option>').trigger('change');
                        } else {
                            $('select[name="kode_pju"]').append('<option value="' +
                                value.kode_pju + '">' + value.kode_pju +
                                '</option>');
                        }
                    });
